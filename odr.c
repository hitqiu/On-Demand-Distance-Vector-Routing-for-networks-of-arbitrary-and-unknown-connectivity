		...
		if (FD_ISSET(unxsock,&rset)) {
			...
			if ((route == 0) || (discovery == 1) || (stale == 1)) {
				...
				printf(
						...
						inet_ntop(AF_INET, &(destad.sin_addr), bufad, 16));
				//bid++;
				printf("The client odr is broadcasting to find the route to the destination\n");
				...
				printf("The application has the route to the destination\n");
...

			{
				...
				display_rtable(&rhead);
				if (route1 == 1) {
					printf("A route to the destination is found in the routing table\n");
				...
				{
					printf("The routing entry has become stale\n");
					stale1 = 1;
				}
				else
					stale1 = 0;
				...
				if ((ntohl(odr2->rq.dstaddr.s_addr) == myaddr.s_addr)
					...
					if (ntohl(odr2->rq.dstaddr.s_addr) == myaddr.s_addr) {
						printf("The destination of the rreq received rreq\n");
						...
						if (ispro1 == 1) {
							...
							printf("I have already seen this source\n");
						...
						if ((ispro1 == 0) || (hopflag1 == 1)
							...
							if (((ispro1 == 0) || (hopflag1 == 1))
								...
								for (ifi = ifihead; ifi != NULL ;
										ifi = ifi->next) {
									printf("flooding rreq by the destination\n");
						...
						printf("intermediate node received rreq and can generate rrep\n");
						ispro1 = isprocessed(&proreq, NTOHLSV(odr2->rq.srcaddr),
								ntohl(odr2->rq.broadcastid));
						if (ispro1 == 1) {
							//tablehopcnt=gethopcount(&proreq,odr2->rq->srcaddr,odr2->rq->broadcastid);
						...
						if (((ispro1 == 0) || (hopflag1 == 1))
							...
							if (ntohl(odr2->rq.rrepsent) == 0) {
								printf(" Intermediate node sending rrep\n");
								memset(buff2, 0, ETH_FRAME_LEN);
								etherhead = buff2;
								eh = (struct ethhdr *) etherhead;
								memset(&socket_address3, 0,
							...
							memset(buffer1, 0, ETH_FRAME_LEN);
							for (ifi = ifihead; ifi != NULL ; ifi = ifi->next) {
								printf("intermediate node broadcasting rreq as it is a better route or previously unknown source\n");
								if (ifi->indx == interface_index)
									continue;
								etherhead1 = buffer1;
								eh1 = (struct ethhdr *) etherhead1;
					...
					if ((route1 == 0) || (stale1 == 1)
						...
						if ((ispro == 0) || (hopflag == 1)) {
							...
							memset(buffer, 0, ETH_FRAME_LEN);
							printf("intermediate node broadcasting rreq as it does not have a route\n");
							for (ifi = ifihead; ifi != NULL ; ifi = ifi->next) {
				...
				if ((ntohl(odr2->rp.dstaddr.s_addr) == myaddr.s_addr)
					...
					if (ntohl(odr2->rp.dstaddr.s_addr) == myaddr.s_addr) {
						...
						printf("original sender odr received rrep\n");
						tablehopcnt = gethopcount(&rhead,
						...
						if (hopflag2 == 1) {
							printf("intermediate node relaying rrep\n");
					...
					printf("Intermediate node received rrep but does not have the backward path,so broadcasting\n");
					if ((tablehopcnt <= ntohl(odr2->rp.hopcnt))
							|| (ntohl(odr2->rp.forceddiscovery) == 1)
							|| (st == 1)) {
						hopflag2 = 1;
					...
					if (hopflag2 == 1) {
						...
						printf("intermediate node initiating rreq\n");
						for (ifi = ifihead; ifi != NULL ; ifi = ifi->next) {
				...
				if ((ntohl(odr2->pl.destaddr.s_addr) == myaddr.s_addr)
					...
					if (ntohl(odr2->pl.destaddr.s_addr) == myaddr.s_addr) {
						...
						/*ispro1=isrepprocessed(&prorep,odr2->rp->srcaddr,odr2->rp->broadcastid);
						 ...
						 {*/
						//tablehopcnt=gethopcountrep(&prorep,odr2->rp->srcaddr,odr2->rp->broadcastid);
						printf("final destination received payload\n");
						...
						etherhead = buff5;
						eh = (struct ethhdr *) etherhead;
						printf("intermediate node relaying payload\n");
					...
					printf("intermediate node receives payload,does not have forward path,thus broadcasts");
					odr2->pl.hopcnt = htonl(ntohl(odr2->pl.hopcnt) + 1);
					tablehopcnt = gethopcount(&rhead,
							NTOHLSV(odr2->pl.srcaddr));
					if (tablehopcnt != 0) {
					...
					display_ptable(&pdata);
					memset(buff4, 0, ETH_FRAME_LEN);
					printf("Intermediate node storing payload n rebroadcasting as it does not have route\n");
					for (ifi = ifihead; ifi != NULL ; ifi = ifi->next) {
