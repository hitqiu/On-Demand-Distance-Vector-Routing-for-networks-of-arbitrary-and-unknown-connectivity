...
void disply(struct ifiinfo **disp_head)
{
	...
	while(temp!=NULL)
	{
		printf("the ifiinfo structure consists of *****************************\n");
		printf("index= %d \n",(temp->indx));
		printf("         HW addr = \n");
		ptr =	temp->hwaddr;
		i = 6;
...
void display_rtable(struct routingtable **drt_phead)
{
	...
	while(temp!=NULL)
	{
		...
		//printf("nexthop: %s\n",temp->nexthop);
		printf("dest addr %s \n ",inet_ntop(AF_INET,&(temp->dest),destaddr1,16));
		//printf("dest %ud \n",temp->dest.s_addr);
		printf("hopcount %d \n",temp->hopcnt);
		printf("index: %d \n",temp->ifiindex);
		printf("time : %lu \n",temp->timestamp);
...
void display_ptable(struct pendingdata **pdrt_phead)
{
	...
	while(temp!=NULL)
	{
		...
		//printf("dest %u \n",temp->destaddr);
		printf("dest addr %s \n ",inet_ntop(AF_INET,&(temp->destaddr),destaddr1,16));
		printf("destport %d \n",temp->destport);
		printf("hopcount %d \n",temp->hopcnt);
		strcpy(recvdata,temp->msg);
		printf("message : %s \n",recvdata);
		//printf("msg: %s \n",temp->msg);
		printf("bytes %d\n",temp->bytes);
		//printf("src addr : %u \n",temp->srcaddr);
		printf("source addr %s \n ",inet_ntop(AF_INET,&(temp->srcaddr),srcaddr1,16));
		printf("src port : %d \n",temp->srcport);
		temp=temp->next;
	}
}
...
void display_rreqtable(struct rreqslist **dpsrreq_phead)
{
	...
	while(temp!=NULL)
	{
		...
		//printf("src %u \n",temp->src);
		printf("************************************\n");
		printf("src addr %s \n ",inet_ntop(AF_INET,&(temp->src),destaddr1,16));
		//printf("destport %d \n",temp->destport);
		printf("hopcount %d \n",temp->hopcnt);
		//printf("msg: %s\n",temp->msg);
		//printf("bytes %d\n",temp->bytes);
...
{
	struct pendingrrep *temp,*r;
	char destaddr1[400],srcaddr[400];
	r=NULL;
	temp=*prrep_phead;
	while(temp!=NULL)
	{
		printf("the pending rrep  table consists of \n");
		printf("************************************\n");
		printf("src addr %s \n ",inet_ntop(AF_INET,&(temp->srcaddr),srcaddr,16));
		printf("dest addr %s \n ",inet_ntop(AF_INET,&(temp->dstaddr),destaddr1,16));
		//printf("src %u \n",temp->srcaddr);
		//printf("dest %u \n",temp->dstaddr);
		printf("hopcount %d \n",temp->hopcnt);
		//printf("msg: %s\n",temp->msg);
...
struct in_addr HTONLSV(struct in_addr temp)
{
	struct in_addr to_return;
	to_return.s_addr=htonl(temp.s_addr);
	return to_return;
}


struct in_addr NTOHLSV(struct in_addr temp)
{
	struct in_addr to_return;
	to_return.s_addr=ntohl(temp.s_addr);
		return to_return;
}

void printhwaddr(unsigned char hwaddr1[6])
{
	char *ptr;
	int i;
